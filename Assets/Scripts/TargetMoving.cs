...
public class TargetMoving : MonoBehaviour
{
    ...
    void Update()
    {
        if(Input.GetKey(KeyCode.UpArrow)){
            if((obiekt.transform.position + Vector3.forward*Time.deltaTime*movementSpeed).magnitude <= length)
                obiekt.transform.position += Vector3.forward*Time.deltaTime*movementSpeed;
        }
        if(Input.GetKey(KeyCode.DownArrow)){
            if((obiekt.transform.position + -1*Vector3.forward*Time.deltaTime*movementSpeed).magnitude <= length)
                obiekt.transform.position += -1*Vector3.forward*Time.deltaTime*movementSpeed;
        }
        if(Input.GetKey(KeyCode.RightArrow)){
            if(( obiekt.transform.position + Vector3.right*Time.deltaTime*movementSpeed).magnitude <= length)
                obiekt.transform.position += Vector3.right*Time.deltaTime*movementSpeed;
        }
        if(Input.GetKey(KeyCode.LeftArrow)){
            if((obiekt.transform.position + -1*Vector3.right*Time.deltaTime*movementSpeed).magnitude <= length)
                obiekt.transform.position += -1*Vector3.right*Time.deltaTime*movementSpeed;
        }
        if(Input.GetKey(KeyCode.Space)){
            if((obiekt.transform.position + Vector3.up*Time.deltaTime*movementSpeed).magnitude <= length)
